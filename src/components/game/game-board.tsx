...
const GameBoard: React.FC<GameBoardProps> = ({ board, player }) => {
  ...
  player.tetromino.shape.forEach((row, y) => {
    row.forEach((value, x) => {
      if (value !== 0) {
        ...
        if (displayBoard[boardY] && displayBoard[boardY][boardX] && displayBoard[boardY][boardX][1] !== 'merged') {
  ...
  return (
    <div
      className="grid border-2 border-primary/50 bg-black/30 shadow-lg rounded-lg overflow-hidden shadow-[0_0_20px_theme(colors.primary/0.5)] aspect-[1/2] w-full"
      style={{
